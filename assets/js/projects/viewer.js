...
// minimal, deduped viewer. keep exports the same to avoid breaking callers.
  ...
  fetch(svg_path)
    ...
    .then(txt => {
      ...
      wrap.addEventListener('wheel', e => {
        ...
        scale = Math.min(Math.max(scale * p, 0.4), 8);
        apply();
      }, {passive:false});
    ...
    .catch(() => {
      ...
      ph.textContent = 'svg failed to load';
...
/**
 * init the project viewer ui (tabs + tree handlers + default selection)
 * note: expects a global PROJECTS object and .tree-item/.project-details elements in the dom.
 * @param {HTMLElement} stage - container where content renders
 * @param {HTMLElement} tabsBar - element that holds tab buttons
 * @param {string} [defaultProject='dgai'] - project id to open on load
 */
export function init_project_viewer(stage, tabsBar, defaultProject = 'dgai'){
  // tab/body render
      ...
      fetch(cfg.script)
        ...
        .then(txt => {
          ...
          Object.assign(pre.style, {
            margin:0, padding:'14px', background:'#0a0f1e',
            color:'#fff', fontSize:'.8rem', lineHeight:'1.4',
